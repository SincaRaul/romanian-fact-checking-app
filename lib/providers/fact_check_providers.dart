...
final homeStatisticsProvider = FutureProvider<Map<String, dynamic>>((
  ref,
) async {
  final apiService = ref.watch(apiServiceProvider);
  ...
  try {
    ...
    try {
      ...
      final autoGenerated = allFactChecks
          .where((fc) => fc.autoGenerated)
          .length;
      ...
      return {'total': 0, 'autoGenerated': 0, 'manual': 0};
    }
  }
});
...
final userAnalyticsProvider = FutureProvider<Map<String, dynamic>>((ref) async {
  ...
  try {
    // Get the anonymous user ID for tracking
    final uid = await analytics.getAnonId();

    // Call the user analytics endpoint
    final response = await apiService.dio.get('/user-analytics/$uid');
    return Map<String, dynamic>.from(response.data);
  } catch (e) {
    // Return fallback stats if API call fails
    return {'stories_read': 0, 'stories_shared': 0, 'questions_submitted': 0};
  }
});
